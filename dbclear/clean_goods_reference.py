...
cnt = 0;
sql = "select gr.id from tb_goods_reference gr left join tb_goods g on gr.goods_id = g.id where g.id is null and gr.id >= %s and gr.id <= %s";
delsql = "delete from tb_goods_reference where id in (%s)";
while (True and statrId < endId) :
    ...
    if len(ids) > 0:
        ...
        res = cursor.rowcount
        cnt = cnt + res;
    print("删除数据goods_reference, startId: %s, endId: %s, num: %s" % (statrId, endIdC, res))

    statrId = statrId + step;
    db.autocommit(False);
    db.commit()

print("删除goods_reference数据条数：%s" % cnt)
# 关闭数据库连接
db.close()
