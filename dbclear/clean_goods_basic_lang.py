...
cnt = 0;
sql = "select gbr.id from tb_goods_basic_lang gbr left join tb_goods_basic gb on gbr.basic_id = gb.id where gb.id is null and gbr.id >= %s and gbr.id <= %s";
...
while (True and statrId < endId) :
    ...
    if len(ids) > 0:
        ...
        res = cursor.rowcount
        cnt = cnt + res;
    print("删除数据goods_basic_lang, startId: %s, endId: %s, num: %s" % (statrId, endIdC, res))

    statrId = statrId + step;
    db.autocommit(False);
    db.commit()

print("删除goods_basic_lang数据条数：%s" % cnt)
# 关闭数据库连接
db.close()
